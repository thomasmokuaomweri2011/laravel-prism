        ...
        <div class="flex-1 overflow-y-auto p-8 space-y-6 bg-white/70">
            @forelse(($brochures ?? []) as $i => $b)
                <div class="p-6 rounded-2xl border border-gray-200 bg-white/80 shadow-sm hover:shadow-md transition-all">
                    ...
                    <!-- Loop dynamically through languages -->
                    @php
                        $languages = collect($b)->except(['url', 'logo'])->toArray();
                        $flags = [
                            'english' => '🇬🇧',
                            'swahili' => '🇰🇪',
                            'french'  => '🇫🇷',
                        ...
                        <div class="mb-10">
                            ...
                            <div class="space-y-2">
                                ...
                                @if(!empty($data['tagline']))
                                    <p class="text-gray-600 italic">"{{ $data['tagline'] }}"</p>
                                @endif

                                @if(!empty($data['mission']))
                                    <p class="text-gray-700"><strong>Mission:</strong> {{ $data['mission'] }}</p>
                                @endif
                    ...
                    <div class="mt-6 flex justify-between items-center">
                        <a href="{{ $b['url'] ?? '#' }}" target="_blank" class="text-blue-600 hover:underline text-sm">
                            Visit Website
                        </a>
